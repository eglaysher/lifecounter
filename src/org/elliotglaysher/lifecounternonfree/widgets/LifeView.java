...
public class LifeView extends View {
    ...
    private void buildPaints() {
        ...
        textPaint = new Paint();
        textPaint.setTextSize(res.getDimension(R.dimen.scrollback_font_size));
    ...
    protected void onDraw(Canvas canvas) {
        RectF bounds = new RectF(getPaddingLeft(), getPaddingTop(), getWidth()
        ...
        canvas.drawRoundRect(bounds, CORNER_RADIUS, CORNER_RADIUS,
                backgroundPaint);
        canvas
                .drawRoundRect(bounds, CORNER_RADIUS, CORNER_RADIUS,
                        outlinePaint);

        if (model != null) {
            FontMetrics fm = textPaint.getFontMetrics();
            final float textOffset = Math.abs(fm.ascent) + Math.abs(fm.descent)
                    + Math.abs(fm.leading);
            ...
            final float startDrawY = bounds.top + Math.abs(fm.ascent)
                    + columnCenteringOffset;
            ...
            while (i < history.size()) {
                ...
                int endOfCol = Math.min(columnStart + entriesInAColumn, history
                        .size());
                for (int j = i; j < endOfCol; ++j) {
                    maxWidth = Math.max(maxWidth, textPaint.measureText(history
                            .get(j)));
                }
            ...
            // Step 3: Render!
            for (int columnNum = startRenderingCol; columnNum < columns.size(); columnNum++) {
